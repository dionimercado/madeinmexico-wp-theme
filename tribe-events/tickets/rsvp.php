...
?>
<form action="" class="cart <?php echo esc_attr( $messages_class ); ?>" method="post" enctype='multipart/form-data' style="background-color: #f1f1f1; padding: 20px;">
	<h2 class="tribe-events-tickets-title" style="margin: 0 0 7px 0;
    padding: 0;
    text-align: center;
    font-family: 'Alfa Slab One', cursive;
    color: #016344;
    letter-spacing: 5px;"><?php echo esc_html_x( 'RSVP', 'form heading', 'event-tickets' ) ?></h2>
	...
	<table width="100%" class="tribe-events-tickets tribe-events-tickets-rsvp">
		...
		?>
		<tr>
			<td style="padding: 0;" class="tribe-ticket quantity" data-product-id="<?php echo esc_attr( $ticket->ID ); ?>">
        <div class="guacamole-rsvp">
          <h4 class="ticket-name"><?php echo esc_html( $ticket->name ); ?></h4>
          <small><?php echo esc_html( $ticket->description ); ?></small>
  				...
  				<?php if ( $is_there_any_product_to_sell ): ?>
  					...
  					<?php endif; ?>
  				<?php else: ?>
  					<span class="tickets_nostock"><?php esc_html_e( 'Sold Out!', 'event-tickets' ); ?></span>
  				<?php endif; ?>
		...
		<?php if ( $is_there_any_product_to_sell ): ?>
			<tr class="tribe-tickets-meta-row">
				<td colspan="4" class="tribe-tickets-attendees" style="padding: 0;">
          <div style="background-color: #016344; color: #fff; padding-bottom: 12px;">
            <header style="background-color: rgba(255,255,255,.08);
    text-transform: uppercase;
    text-align: center;
    padding: 7px;
    margin-bottom: 10px;"><?php esc_html_e( 'Send RSVP confirmation to:', 'event-tickets' ); ?></header>
  					<?php
  					/**
  					...
  					<table class="tribe-tickets-table">
  						<tr class="tribe-tickets-full-name-row">
  							...
  							<td colspan="3">
  								<input type="text" name="attendee[full_name]" id="tribe-tickets-full-name" class="tribe-ticket-quantity">
  							</td>
  						</tr>
              <tr class="tribe-tickets-email-row">
  							...
  							<td colspan="3">
  								<input type="email" name="attendee[email]" id="tribe-tickets-email" class="tribe-ticket-quantity">
  							</td>
  						</tr>
              <tr class="tribe-tickets-phone-row">
  							<td>
  								<label for="tribe-tickets-phone"><?php esc_html_e( 'Phone', 'event-tickets' ); ?>:</label>
  							</td>
  							<td colspan="3">
  								<input type="phone" name="attendee[phone]" id="tribe-tickets-email" class="tribe-ticket-quantity">
  							</td>
  						</tr>
			...
			<tr>
				<td colspan="4" class="add-to-cart" style="padding: 0;">
					<?php if ( $must_login ): ?>
						<a href="<?php echo Tribe__Tickets__Tickets::get_login_url(); ?>"><?php esc_html_e( 'Login to RSVP', 'event-tickets' );?></a>
					<?php else: ?>
						<button style="margin-top: 20px;" type="submit" name="tickets_process" value="1" class="button alt gform_button"><?php esc_html_e( 'Confirm RSVP', 'event-tickets' );?></button>
					<?php endif; ?>
				</td>
			</tr>
		<?php endif; ?>
